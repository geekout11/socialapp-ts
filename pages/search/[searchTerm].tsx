...
const Search = ({ videos }: { videos: Video[] }) => {
  ...
  const { allUsers }: { allUsers: UserProps[] } = useAuthStore();
  ...
  const searchedAccounts = allUsers?.filter((user: UserProps) => user.userName.toLowerCase().includes(searchTerm));
      ...
      {isAccounts ? (
        <div className='md:mt-16'>
          {searchedAccounts.length > 0 ? (
            searchedAccounts.map((user: UserProps, idx: number) => (
